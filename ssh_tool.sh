...
case $choice in
    ...
    while true; do
      ...
      case $sub_choice in
          1)
              clear
              install wget
              wget https://raw.githubusercontent.com/ReapproveSpark/tools/refs/heads/main/sh/magic.sh && chmod +x magic.sh && ./magic.sh
              rm magic.sh
            ;;
          2)
              clear
              install wget
              wget -q https://raw.githubusercontent.com/ReapproveSpark/tools/refs/heads/main/sh/ssh.sh && chmod +x  ssh.sh && ./ssh.sh
              rm ssh.sh
            ;;
          3)
              clear
         ...
         8)
          ...
          10)
              ...
              docker run -d -v=/root/cgent/:/root/ --name=cgent --restart=always --net=host --cap-add=NET_RAW -e SECRET=8vb0R7wuNjrXdxZgkrNQAgsRhfyhtesF -e SERVER=nezha.buzhi.de:443 -e TLS=true ghcr.io/yosebyte/cgent
            ;;
          11)
              clear
  ...
  00)
    cd ~
    rm ssh_tool.sh
    curl -fsSL https://raw.githubusercontent.com/ReapproveSpark/tools/refs/heads/main/ssh_tool.sh -o ssh_tool.sh && chmod +x ssh_tool.sh
    sleep 1
